...
### How to Update Your Fork with the Original Repository

To keep your fork up to date with the latest changes from the original repository:
1. Add the original repository as an upstream remote:
   ```bash
   git remote add upstream git@github.com:firozfau/made-template.git
   ```
2. Fetch the changes from the original repository:
   ```bash
   git fetch upstream
   ```
3. Merge the changes into your fork’s `main` branch:
   ```bash
   git checkout main
   git merge upstream/main
   ```
4. Push the updated code to your forked repository on GitHub:
   ```bash
